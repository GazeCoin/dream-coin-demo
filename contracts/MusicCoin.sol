 ...
 * @dev ERC20 token
 */
contract MusicCoin is Ownable {
  using SafeMath for uint256;

  event Transfer(address indexed _from, address indexed _to, uint256 _value);
  event Approval(address indexed _owner, address indexed _spender, uint256 _value);
  
  event Buy(address indexed _from, uint256 _etherReceived, uint256 _tokensSent);
  ...
  //amount of tokens for one ether
  uint256 public rate;
  ...
  /**
   * @dev This method accepts ether and isses adequate amount of tokens back
   */
  function buy() public payable {
    uint256 tokensDue = msg.value.mul(rate);
    wallet.transfer(msg.value);
    mint(msg.sender, tokensDue);
    emit Buy(msg.sender, msg.value, tokensDue);
  }
  ...
  */
   ...
   */
  function approveAndCall(address _recipient, uint256 _value, bytes _data) public returns (bool) {
